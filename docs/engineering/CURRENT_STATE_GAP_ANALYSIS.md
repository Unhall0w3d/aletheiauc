...
| CUC/CUCM configuration depth | Implemented; continued cross-version validation required | Bounded CUPI configuration GETs and extended AXL hunt, configured CFA, SIP-destination, line-group membership, integration-security, LDAP, and media-resource discovery feed dedicated report tables. The 2026-07-13 CUCM 15/CUC 15 review validated live field aliases, added bounded message-aging child rules, exposed partial CUPI coverage, recognized UUID-only line-group members, and added fixed SQL recovery for line-group DNs and SIP targets. Diagnostic CUC now includes three publisher-only experimental `SELECT FIRST 100` Informix probes with strict validation and reporting; mailbox databases remain excluded pending safe load evidence. | validation |
...
- Experimental CUC Informix schema compatibility, fixed-width parsing, runtime,
  and zero-row behavior across supported versions and representative deployments.
- CUPI field-name and endpoint variants across supported CUC versions, including
  message-aging child rules and SMTP availability before 14SU2.
